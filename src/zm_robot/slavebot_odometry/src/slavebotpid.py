...
if __name__ == '__main__':
    rospy.init_node('slavebotpid', anonymous=True)    
    rospy.Subscriber('/slave_cmd_vel', Twist, cmd_vel_callback) 
    rospy.Subscriber("/slave_odom", Odometry, odom_callback)
    rospy.Subscriber('/sprop', Float32, propCallback)
    rospy.Subscriber('/sinte', Float32, inteCallback)

    twist_pub = rospy.Publisher('slave_pid_vel', Twist, queue_size=10)
    current_lvel = rospy.Publisher('scurrent_lvel', Float32, queue_size=1)
    target_v = rospy.Publisher('starget_v', Float32, queue_size=1)
