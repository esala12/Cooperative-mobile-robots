...
class TargetVelocityController:
    ...
    def generate_path(self):
        # Generate path points (same as original)
        diameter1 = 3
        radius1 = diameter1 / 2
        ...
        path1 = np.column_stack((np.linspace(3, -7, num_points), np.full(num_points, -1)))
    ...
    def update_goal_master(self, distance_x, distance_y):
        ...
        if self.forward_slave_goal and distance_to_end <= self.distance_master:
            # Stop updating master goal when within self.distance_master from the end of path
            rospy.loginfo("Master is within distance_master of the end of the path. Keeping current goal.")
            self.reached_final_waypoint = True  # Stop updating the goal
            return  # Do not update goal_master, keep it at the current position
