...
class TargetVelocityController:
    def __init__(self):
     ...
     #    points4[:, 0] += amplitude * np.sin(frequency * points4[:, 1])
        ...
        # Publishers
        self.error_x_pub = rospy.Publisher('error_slave_x', Float64, queue_size=1)
        self.input_linear_vel_pub = rospy.Publisher('input_linear_slave_x_vel', Float64, queue_size=1)
        self.error_y_pub = rospy.Publisher('error_slave_y', Float64, queue_size=1)
        self.input_linear_y_vel_pub = rospy.Publisher('input_linear_slave_y_vel', Float64, queue_size=1)
        self.vel_pub = rospy.Publisher('slave_cmd_vel', Twist, queue_size=10)
...
if __name__ == '__main__':
    rospy.init_node('fuzzy_matlab_slave_node')

    fuzzy_controller = TargetVelocityController()

    rospy.Subscriber('slavebot_kalman', Odometry, fuzzy_controller.odom_callback)
    rospy.Subscriber('output_linear_slave_x_vel', Float64, fuzzy_controller.vel_x_callback)
    rospy.Subscriber('output_linear_slave_y_vel', Float64, fuzzy_controller.vel_y_callback)
